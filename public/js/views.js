        ...
        <div className="w-full max-w-3xl mx-auto text-center fade-in pb-10">
            <div className="bg-white dark:bg-slate-800 rounded-3xl p-10 shadow-2xl border border-slate-200 dark:border-slate-700 transition-colors duration-300 mb-8">
                ...
                <div className="flex justify-center gap-3 mb-6">
                    <button onClick={handleDownload} className="px-4 py-2 bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-xl font-bold text-sm hover:bg-indigo-50 dark:hover:bg-indigo-900/20 hover:text-indigo-500 dark:hover:text-indigo-400 transition flex items-center gap-2 active:scale-95">
                        {/* 🔥 Fixed Icon */}
                        <Icons.Download /> PDF
                    </button>
                    <button onClick={handleShare} className="px-4 py-2 bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-xl font-bold text-sm hover:bg-indigo-50 dark:hover:bg-indigo-900/20 hover:text-indigo-500 dark:hover:text-indigo-400 transition flex items-center gap-2 active:scale-95">
                        <Icons.Share /> Share
                    </button>
                </div>

                <div className="space-y-3 max-w-md mx-auto"><button onClick={onRetry} className="w-full py-4 bg-indigo-600 dark:bg-[#10b981] text-white dark:text-slate-900 rounded-xl font-bold shadow-lg hover:opacity-90 transition active:scale-95">{isReviewMode ? "Back" : "New Quiz"}</button>{!user && !isReviewMode && <button onClick={onOpenLogin} className="w-full py-4 bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-xl font-bold hover:bg-slate-200 dark:hover:bg-slate-600 transition flex items-center justify-center gap-2 active:scale-95">Save My Progress <Icons.ArrowRight /></button>}{user && !isReviewMode && (<div className="text-xs text-green-500 font-bold uppercase tracking-widest mt-4">✓ Result Saved</div>)}</div>
            </div>
            <div className="text-left space-y-6">{quizData.map((q, i) => { const usr = userAnswers[i]; const cor = q.correctAnswer; const ok = usr===cor; return <div key={i} className="bg-white dark:bg-slate-800 rounded-2xl p-6 border border-slate-200 dark:border-slate-700 shadow-sm hover:scale-[1.01] transition-transform duration-200"><div className="flex items-start gap-4"><div className={`mt-1 w-8 h-8 rounded-full flex items-center justify-center shrink-0 font-bold text-sm ${ok ? 'bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400' : 'bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400'}`}>{ok ? <Icons.Check /> : <Icons.Cross />}</div><div className="w-full"><h4 className="text-lg font-bold text-slate-900 dark:text-white mb-3">{q.question}</h4><div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4"><div className={`p-3 rounded-lg text-sm border flex gap-3 items-center ${ok ? 'bg-green-50 dark:bg-green-900/10 border-green-200 dark:border-green-900 text-green-800 dark:text-green-400' : 'bg-red-50 dark:bg-red-900/10 border-red-200 dark:border-red-900 text-red-800 dark:text-red-400'}`}><div className="font-bold px-2 py-1 rounded bg-white/50 dark:bg-black/20">{usr||"-"}</div><span>Your Answer</span></div>{!ok && <div className="p-3 rounded-lg text-sm bg-slate-50 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 text-slate-700 dark:text-slate-300 flex gap-3 items-center"><div className="font-bold px-2 py-1 rounded bg-indigo-100 dark:bg-indigo-900 text-indigo-600 dark:text-indigo-400">{cor}</div><span>Correct</span></div>}</div><div className="bg-indigo-50 dark:bg-slate-900/50 p-4 rounded-xl border border-indigo-100 dark:border-slate-700"><span className="text-[10px] font-bold text-indigo-600 dark:text-[#10b981] uppercase tracking-wider block mb-2">Theory</span><p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed">{q.explanation}</p></div></div></div></div> })}</div>
        </div>
    );
});