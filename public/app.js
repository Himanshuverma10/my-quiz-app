    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm fade-in p-4">
            <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-700 p-8 max-w-md w-full text-center relative">
                ...
                <p className="text-slate-500 dark:text-slate-400 mb-8">Log in to track your quiz history and analytics forever.</p>
...
// --- SETUP VIEW (Modified for Guest) ---
const SetupView = ({ user, onGenerate, loading, error, progressText, onLogout, onOpenLogin }) => {
    ...
    return (
        <div className="w-full max-w-5xl mx-auto fade-in">
            <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-700 transition-all duration-300">
                ...
                {/* Header: Handles both Guest and User */}
                    ...
                    <div className="hidden md:flex bg-slate-50 dark:bg-slate-900/50 p-10 flex-col justify-center relative overflow-hidden border-l border-slate-200 dark:border-slate-700 transition-colors">
                        ...
                        {user ? (
                            ...
                            <div className="relative z-10 text-center">
                                ...
                                <div className="p-4 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm mb-4">
                                    <p className="text-xs text-slate-400 uppercase font-bold">Quizzes Taken</p>
                                    <p className="text-2xl font-black text-indigo-600 dark:text-[#10b981]">0</p>
                                </div>
                                <p className="text-slate-500 text-xs">Generate a quiz to start tracking.</p>
                            </div>
                        ) : (
                            // GUEST VIEW (LOCKED)
                            <div className="relative z-10 text-center opacity-70">
                                ...
                                <p className="text-slate-500 text-sm mb-6 max-w-xs mx-auto">Log in to track your progress, save subjects, and see analytics.</p>
                                <button onClick={onOpenLogin} className="text-indigo-600 dark:text-[#10b981] font-bold text-sm hover:underline">
                                    Login to Unlock
                                </button>
                            </div>
                        )}
...
// --- RESULT VIEW (Guest Logic Added) ---
const ResultView = ({ score, total, user, onRetry, onOpenLogin }) => {
        ...
        <div className="w-full max-w-md mx-auto text-center fade-in">
            <div className="bg-white dark:bg-slate-800 rounded-3xl p-10 shadow-2xl border border-slate-200 dark:border-slate-700 transition-colors duration-300">
                ...
                <div className="space-y-3">
                    ...
                    {/* Show login prompt if user is a guest */}
                    {!user && (
                        <button onClick={onOpenLogin} className="w-full py-4 bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-xl font-bold hover:bg-slate-200 dark:hover:bg-slate-600 transition flex items-center justify-center gap-2">
...
const App = () => {
    ...
    useEffect(() => {
        ...
        const unsubscribe = auth.onAuthStateChanged((u) => {
            setUser(u);
            if (u) setShowLoginModal(false); // Close modal on success
        });
        return unsubscribe;
    }, []);
        ...
        <div className={theme}>
            <div className="min-h-screen w-full bg-slate-100 dark:bg-[#0f172a] text-slate-900 dark:text-white transition-colors duration-500 flex flex-col p-4 md:p-8 relative font-sans">
                ...
                <div className="flex-1 flex items-center justify-center">
                    {/* 🚀 LOGIC CHANGE: Always render Views, just pass 'user' prop down */}
                    {view === 'setup' && (
                        <SetupView 
                            user={user} 
