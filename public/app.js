...
const isOptionCorrect = (option, correctKey) => {
    ...
    // Prefix Match (e.g. "A) Text" starts with "A)")
...
const EditSubjectModal = ({ user, subject, onClose, onUpdate }) => {
    ...
    const handleSave = async () => {
        setParsing(true);
        if(name !== subject.name) { await db.collection('users').doc(user.uid).collection('subjects').doc(subject.id).update({ name }); }
        ...
        setParsing(false); onUpdate(); onClose();
    };
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm fade-in p-4">
            <div className="bg-white dark:bg-slate-800 rounded-3xl p-8 max-w-2xl w-full shadow-2xl border border-slate-200 dark:border-slate-700 max-h-[90vh] overflow-y-auto">
                ...
                <div className="mb-6">
                    <label className="block text-xs font-bold text-slate-400 uppercase mb-2">Add New Content</label>
...
const SubjectDetail = ({ subject, topics, onBack, onStartTopic, onLearnTopic }) => {
            ...
            <div className="w-full max-w-4xl mx-auto fade-in p-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {units.map((unit) => {
                        ...
                        return (
                            <div key={unit.name} onClick={() => setSelectedUnit(unit)} className={`bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border ${isMastered ? 'border-yellow-500/50 bg-yellow-500/5' : 'border-slate-200 dark:border-slate-700'} cursor-pointer hover:border-indigo-500 transition group relative overflow-hidden`}>
    ...
    // View: Topics inside selected Unit
    return (
        <div className="w-full max-w-4xl mx-auto fade-in p-4">
                ...
                {selectedUnit.topics.map((topic, index) => (
                    <div key={index} className="flex flex-col md:flex-row md:items-center justify-between p-5 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 hover:border-indigo-400 transition gap-4">
                        ...
                        <div className="flex gap-2 w-full md:w-auto">
                            <button onClick={() => onLearnTopic(topic.name, subject.name, topic.id)} className={`flex-1 md:flex-none px-4 py-2 rounded-lg font-bold text-sm transition flex items-center justify-center gap-2 ${topic.learned ? 'bg-blue-100 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400' : 'bg-indigo-50 text-indigo-600 dark:bg-indigo-900/20 dark:text-indigo-400 hover:bg-indigo-100'}`}>{topic.learned && <Icons.Check />} Learn</button>
                            <button onClick={() => onStartTopic(topic.name)} className={`flex-1 md:flex-none px-4 py-2 rounded-lg font-bold text-sm transition flex items-center justify-center gap-2 ${topic.completed ? 'bg-green-100 text-green-600 dark:bg-green-900/30 dark:text-green-400' : 'bg-indigo-600 text-white hover:bg-indigo-700'}`}>{topic.completed ? <><Icons.CheckCircle /> {topic.score}%</> : <><Icons.Play /> Quiz</>}</button>
                        </div>
                    </div>
                ))}
            </div>
...
const App = () => {
    ...
    const handleLearnInit = (topicName, subjectName) => {
        setLearnInitData({ topic: topicName, subject: subjectName });
        setActiveTab('learn');
        setView('home');
    };
    ...
    const generateQuiz = async ({ mode, topic, sourceText, difficulty, numQuestions }) => {
        setLoading(true); setError(''); setCurrentTopicName(mode === 'topic' ? topic : "File");
        try {
